import React from 'react';
import {
  Box,
  Button,
  Container,
  Heading,
  Text,
  VStack,
  HStack,
  Image,
  Flex,
  Link,
} from '@chakra-ui/react';
import { RiUserStarFill, RiSettings4Fill, RiShieldStarFill } from 'react-icons/ri';
import { FaGithub, FaYoutube, FaLinkedin, FaEnvelope, FaPhone } from 'react-icons/fa';

const Feature = ({ icon, title, description }) => (
  <VStack align="center">
    {icon}
    <Heading size="md">{title}</Heading>
    <Text textAlign="center">{description}</Text>
  </VStack>
);

const HomePage = () => {
  return (
    <Box p="4">
      <Container maxW="container.xl">
        <Flex
          direction={['column', 'column', 'row']}
          align="center"
          justify="center"
          minH="100vh"
          py="16"
        >
          <Flex
            direction="column"
            align={['center', 'center', 'flex-start']}
            justify="center"
            flex="1"
            mb={[8, 8, 0]}
            pr={[0, 0, 8]}
          >
            <Heading as="h1" size="2xl" textAlign={['center', 'center', 'left']} mb="4">
              Welcome to My Portfolio
            </Heading>

            <Text fontSize="lg" textAlign={['center', 'center', 'left']} mb="8">
              Driven software engineer proficient in Java, and React.js, with strong expertise in Data Structures, Algorithms, and OOP. Experienced in developing RESTful APIs, backend systems with Spring Boot, and dynamic UIs with Chakra UI. Passionate about creating user-friendly applications, and innovative tech solutions.
            </Text>

            <Text fontSize="lg" textAlign={['center', 'center', 'left']} mb="8">
              Discover my projects and skills. <br />
              Check out my YouTube channel to explore my Data Structures and Algorithms expertise!
            </Text>

            <HStack spacing="4" mb="16" justifyContent={['center', 'center', 'flex-start']}>
              <a href=" https://github.com/Ak-Sat09/" target="_blank" rel="noopener noreferrer">
                <Button colorScheme="teal" size="lg" leftIcon={<FaGithub />}>
                  View on GitHub
                </Button>
              </a>
              <a href="https://www.youtube.com/@akumg09" target="_blank" rel="noopener noreferrer">
                <Button colorScheme="teal" size="lg" leftIcon={<FaYoutube />}>
                  View on YouTube
                </Button>
              </a>
              <a href="https://www.linkedin.com/in/anmehla09" target="_blank" rel="noopener noreferrer">
                <Button colorScheme="teal" size="lg" leftIcon={<FaLinkedin />}>
                  Visit LinkedIn
                </Button>
              </a>
            </HStack>
          </Flex>

          <Image
            src="data:image/jpeg;base64, "
            alt="Profile"
            borderRadius="full"
            boxSize="200px"
          />
        </Flex>
        <Flex direction="column" align="center" py="16">
          <Heading as="h2" size="xl" mb="8">
            Features
          </Heading>

          <HStack spacing="8">
            <Feature
              icon={<RiUserStarFill size="48px" />}
              title="About Me"
              description="Get to know more about me."
              link="/about"
            />
            <Feature
              icon={<RiSettings4Fill size="48px" />}
              title="Skills"
              description="Explore my technical skills."
              link="/skills"
            />
            <Feature
              icon={<RiShieldStarFill size="48px" />}
              title="Experience"
              description="Check out my professional journey."
              link="/experience"
            />
          </HStack>
        </Flex>
        <Footer />
      </Container>
    </Box>
  );
};

const Footer = () => {
  return (
    <Box bg="gray.800" color="white" py="4" mt="8">
      <Container maxW="container.xl">
        <Flex direction="column" align="center">
          <Text fontSize="lg" mb="4">Contact Me</Text>
          <HStack spacing="8">
            <HStack>
              <FaEnvelope />
              <Text>anmolmehla4@gmail.com</Text>
            </HStack>
            <HStack>
              <FaPhone />
              <Text>+91 8398963836</Text>
            </HStack>
          </HStack>
        </Flex>
      </Container>
    </Box>
  );
};

export default HomePage;
